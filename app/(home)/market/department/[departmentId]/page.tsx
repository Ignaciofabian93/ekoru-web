...
export default function BrowseDepartmentResultsPage() {
  ...
  const { products, productsLoading } = useProductsByDepartment({ id: Number(departmentId) });
  ...
  return (
    <PageWrapper>
      <PageHeader image={wallpaper} alt="Portada de departamento" message="Recicla, reutiliza y renueva" />
      <ContentWrapper>
        <Banner
      ...
      <ContentWrapper>
        {productsLoading ? (
          ...
          circularOptionsData &&
          circularOptionsData.length > 0 && <RenderCircularOptions data={circularOptionsData} />
        )}
      </ContentWrapper>
      <ContentWrapper>
        <span>Products</span>
